import torch as th
from torch import nn
from manet.nn.model import MNISTModel

from torch import Tensor
from typing import List, TypeVar, Tuple, Type

import torch._dynamo

torch._dynamo.config.suppress_errors = True


U = TypeVar('U', bound='Unit')


class Unit(nn.Module):
    def __init__(self: U,
                 in_channel: int,
                 out_channel: int,
                 in_spatio: int = 1,
                 out_spatio: int = 1,
                 num_steps: int = 3,
                 step_length: float = 0.33333,
                 num_points: int = 5,
                 ) -> None:
        super().__init__()

        # the hyperparameters
        self.num_steps = num_steps
        self.step_length = step_length
        self.num_points = num_points
        self.in_channel = in_channel
        self.out_channel = out_channel
        self.in_spatio = in_spatio
        self.out_spatio = out_spatio

        self.channel_dim, self.spatio_dim = self.calculate()
        # the learnable parameters which govern the unit
        self.angles = nn.Parameter(
            th.linspace(0, 2 * th.pi, num_points).view(1, 1, num_points)
        )
        self.velocity = nn.Parameter(
            th.linspace(0, 1, num_points).view(1, 1, num_points)
        )

        self.channel_dim, self.spatio_dim = self.calculate()

        self.channel_transform = nn.Parameter(
            th.normal(0, 1, (1, self.in_channel, self.out_channel))
        )
        self.spatio_transform = nn.Parameter(
            th.normal(0, 1, (1, self.in_spatio, self.out_spatio))
        )

    def calculate(self: U) -> Tuple[int, int]:
        raise NotImplemented()

    def expansion(self: U, data: Tensor) -> Tensor:
        raise NotImplemented()

    def reduction(self: U, data: Tensor) -> Tensor:
        raise NotImplemented()

    @th.compile
    def nonlinear(self: U, data: Tensor) -> Tensor:
        for ix in range(self.num_steps):
            data = data + self.step(data) * self.step_length
        return data

    @th.compile
    def accessor(self: U,
                 data: Tensor,
                 ) -> Tuple[Tensor, Tensor, Tensor]:
        # calculate the index of the accessor
        num_points = self.num_points
        index = th.sigmoid(data) * num_points
        bgn = index.floor().long()
        bgn = bgn * (bgn >= 0)
        bgn = bgn * (bgn <= num_points - 2) + (bgn - 1) * (bgn > num_points - 2)
        bgn = bgn * (bgn <= num_points - 2) + (bgn - 1) * (bgn > num_points - 2)
        end = bgn + 1

        return index, bgn, end

    @th.compile
    def access(self: U,
               memory: Tensor,
               accessor: Tuple[Tensor, Tensor, Tensor]
               ) -> Tensor:

        index, bgn, end = accessor
        pos = index - bgn
        memory = memory.flatten(0)
        return (1 - pos) * memory[bgn] + pos * memory[end]

    @th.compile
    def step(self: U,
             data: Tensor
             ) -> Tensor:

        accessor = self.accessor(data)
        angels = self.access(self.angles, accessor)
        velo = self.access(self.velocity, accessor)

        # by the flow equation of the arithmetic expression geometry
        return (velo * th.cos(angels) + data * velo * th.sin(angels)) * self.step_length

    def calculate(self: U) -> Tuple[int, int]:
        channel_dim = self.in_channel * self.out_channel
        spatio_dim = self.in_spatio * self.out_spatio
        self.flag = self.in_channel * self.in_spatio > self.out_channel * self.out_spatio
        return channel_dim, spatio_dim

    @th.compile
    def forward(self: U,
                data: Tensor
                ) -> Tensor:
        data = data.contiguous()

        data = data.view(-1, self.in_channel, self.in_spatio)
        data = th.permute(data, [0, 2, 1]).reshape(-1, self.in_channel)
        data = th.matmul(data, self.channel_transform)
        data = data.view(-1, self.in_spatio, self.out_channel)

        data = self.nonlinear(data)

        data = th.permute(data, [0, 2, 1]).reshape(-1, self.in_spatio)
        data = th.matmul(data, self.spatio_transform)
        data = data.view(-1, self.out_channel, self.out_spatio)

        return data


class MLP(nn.Sequential):
    def __init__(
        self,
        in_channels: int,
        hidden_channels: List[int],
        spatio_dim: int = 1,
        steps: int = 3,
        length: float = 1.0,
        points: int = 5,
        unit: Type[Unit] = Unit
    ) -> None:
        layers = []
        in_dim = in_channels
        for hidden_dim in hidden_channels:
            layers.append(unit(
                in_dim, hidden_dim, spatio_dim, spatio_dim, steps, length / steps, points
            ))
            in_dim = hidden_dim
        layers.append(nn.Flatten())
        super().__init__(*layers)


class Reshape(nn.Module):
    def __init__(self, *shape) -> None:
        super().__init__()
        self.shape = shape

    def forward(self, x):
        return x.view(-1, *self.shape)


class FashionA(MNISTModel):
    def __init__(self):
        super().__init__()
        self.conv0 = nn.Conv2d(1, 5, kernel_size=7, padding=3)
        self.mlpr0 = MLP(1, [1], steps=2, length=1, points=5)
        self.shap0 = Reshape(5, 28, 28)

        self.ncnv1 = nn.Conv2d(5, 5, kernel_size=3, padding=1)
        self.nmlp1 = MLP(1, [1], steps=2, length=1, points=5)
        self.nshp1 = Reshape(5, 28, 28)
        self.ncnv2 = nn.Conv2d(5, 5, kernel_size=3, padding=1)
        self.nmlp2 = MLP(1, [1], steps=2, length=1, points=5)
        self.nshp2 = Reshape(5, 28, 28)
        self.rmlp1 = MLP(1, [1], steps=2, length=1, points=5)
        self.rshp1 = Reshape(5, 28, 28)

        self.conv1 = nn.Conv2d(5, 15, kernel_size=3, padding=1)
        self.mxpl1 = nn.MaxPool2d(2)
        self.mlpr1 = MLP(1, [1], steps=2, length=1, points=5)
        self.shap1 = Reshape(15, 14, 14)

        self.ncnv3 = nn.Conv2d(15, 15, kernel_size=3, padding=1)
        self.nmlp3 = MLP(1, [1], steps=2, length=1, points=5)
        self.nshp3 = Reshape(15, 14, 14)
        self.ncnv4 = nn.Conv2d(15, 15, kernel_size=3, padding=1)
        self.nmlp4 = MLP(1, [1], steps=2, length=1, points=5)
        self.nshp4 = Reshape(15, 14, 14)
        self.rmlp2 = MLP(1, [1], steps=2, length=1, points=5)
        self.rshp2 = Reshape(15, 14, 14)

        self.conv2 = nn.Conv2d(15, 45, kernel_size=3, padding=1)
        self.mxpl2 = nn.MaxPool2d(2)
        self.mlpr2 = MLP(1, [1], steps=2, length=1, points=5)
        self.shap2 = Reshape(45, 7, 7)

        self.ncnv5 = nn.Conv2d(45, 45, kernel_size=3, padding=1)
        self.nmlp5 = MLP(1, [1], steps=2, length=1, points=5)
        self.nshp5 = Reshape(45, 7, 7)
        self.ncnv6 = nn.Conv2d(45, 45, kernel_size=3, padding=1)
        self.nmlp6 = MLP(1, [1], steps=2, length=1, points=5)
        self.nshp6 = Reshape(45, 7, 7)
        self.rmlp3 = MLP(1, [1], steps=2, length=1, points=5)
        self.rshp3 = Reshape(45, 7, 7)

        self.conv3 = nn.Conv2d(45, 135, kernel_size=3, padding=1)
        self.mxpl3 = nn.MaxPool2d(2)
        self.mlpr3 = MLP(1, [1], steps=2, length=1, points=5)
        self.shap3 = Reshape(135, 3, 3)

        self.ncnv7 = nn.Conv2d(135, 135, kernel_size=3, padding=1)
        self.nmlp7 = MLP(1, [1], steps=2, length=1, points=5)
        self.nshp7 = Reshape(135, 3, 3)
        self.ncnv8 = nn.Conv2d(135, 135, kernel_size=3, padding=1)
        self.nmlp8 = MLP(1, [1], steps=2, length=1, points=5)
        self.nshp8 = Reshape(135, 3, 3)
        self.rmlp4 = MLP(1, [1], steps=2, length=1, points=5)
        self.rshp4 = Reshape(135, 3, 3)

        self.avpl4 = nn.AvgPool2d(2)
        self.mlpr4 = MLP(135, [10], steps=2, length=1, points=5)
        self.shap4 = Reshape(10)

        self.lsftx = nn.LogSoftmax(dim=1)

    def forward(self, x):
        y = self.conv0(x)
        y = self.mlpr0(y)
        y = self.shap0(y)

        z = self.ncnv1(y)
        z = self.nmlp1(z)
        z = self.nshp1(z)
        z = self.ncnv2(z)
        z = z + self.rshp1(self.rmlp1(y))
        z = self.nmlp2(z)
        z = self.nshp2(z)

        z = self.conv1(z)
        y = self.mxpl1(z)
        y = self.mlpr1(y)
        y = self.shap1(y)

        z = self.ncnv3(y)
        z = self.nmlp3(z)
        z = self.nshp3(z)
        z = self.ncnv4(z)
        z = z + self.rshp2(self.rmlp2(y))
        z = self.nmlp4(z)
        z = self.nshp4(z)

        z = self.conv2(z)
        y = self.mxpl2(z)
        y = self.mlpr2(y)
        y = self.shap2(y)

        z = self.ncnv5(y)
        z = self.nmlp5(z)
        z = self.nshp5(z)
        z = self.ncnv6(z)
        z = z + self.rshp3(self.rmlp3(y))
        z = self.nmlp6(z)
        z = self.nshp6(z)

        z = self.conv3(z)
        y = self.mxpl3(z)
        y = self.mlpr3(y)
        y = self.shap3(y)

        z = self.ncnv7(y)
        z = self.nmlp7(z)
        z = self.nshp7(z)
        z = self.ncnv8(z)
        z = z + self.rshp4(self.rmlp4(y))
        z = self.nmlp8(z)
        z = self.nshp8(z)

        z = self.avpl4(z)
        z = self.mlpr4(z)
        z = self.shap4(z)

        return self.lsftx(z)


def _model_():
    return FashionA()
