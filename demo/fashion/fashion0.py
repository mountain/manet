...
class Fashion0(MNISTModel):
    def __init__(self):
        super().__init__()
        self.conv0 = nn.Conv2d(1, 14, kernel_size=7, padding=3)
        self.lnon0 = LNon(steps=1, length=1, points=120)
        self.conv1 = nn.Conv2d(14, 52, kernel_size=3, padding=1)
        self.lnon1 = LNon(steps=1, length=1, points=120)
        self.conv2 = nn.Conv2d(52, 156, kernel_size=3, padding=1)
        self.lnon2 = LNon(steps=1, length=1, points=120)
        self.conv3 = nn.Conv2d(156, 468, kernel_size=3, padding=1)
        self.lnon3 = LNon(steps=1, length=1, points=120)
        self.fc = nn.Linear(468 * 9, 10)
